...
async function runMigration() {
  ...
  const hostDisplay = MONGODB_URI.split('@')[1]?.split('?')[0] || 'Atlas';
  console.log(`⏳ Connecting to MongoDB Atlas cluster at: ${hostDisplay}...`);
  ...
  for (const t of sourceTests) {
    try {
      ...
      let difficulty = null;
      if (sanitized.difficulty !== null && sanitized.difficulty !== undefined && String(sanitized.difficulty).trim() !== '') {
        const parsedDiff = parseFloat(sanitized.difficulty);
        if (!isNaN(parsedDiff)) {
